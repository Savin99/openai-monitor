        ...
        "data": [
            {
                "object": "bucket",
                "results": [
                    {"amount": {"value": str(v), "currency": "usd"}} for v in vs
                ],
            }
            for vs in buckets
        ],
...
class StateIsolationMixin:
    ...
    def setUp(self):
        ...
        self._patch_state_file = patch.object(
            utils, "DEFAULT_STATE_FILE", self._state_path
        )
...
class GetCostsForPeriodTests(StateIsolationMixin, unittest.TestCase):
    def test_sums_single_page(self):
        ...
        with patch(
            "openai_monitor.requests.get", return_value=_mock_response(200, page)
        ):
    ...
    def test_paginates_until_no_more(self):
        ...
        with patch(
            "openai_monitor.requests.get",
            side_effect=[_mock_response(200, p1), _mock_response(200, p2)],
        ) as mock_get:
            cost, err = openai_monitor.get_costs_for_period(1000)
    ...
    def test_http_error_all_retries(self):
        ...
        with (
            patch("openai_monitor.requests.get", return_value=err_resp) as mock_get,
    ...
    def test_network_exception_all_retries_fail(self):
        with (
            patch("openai_monitor.requests.get", side_effect=requests.Timeout("slow")),
            patch("openai_monitor.time.sleep"),
        ):
            cost, err = openai_monitor.get_costs_for_period(1000, max_retries=2)
        self.assertIsNone(cost)
    ...
    def test_api_error_in_response_body(self):
        ...
        with patch(
            "openai_monitor.requests.get", return_value=_mock_response(200, body)
        ):
            cost, err = openai_monitor.get_costs_for_period(1000)
        self.assertIsNone(cost)
...
class PeriodHelpersTests(StateIsolationMixin, unittest.TestCase):
    def test_get_total_costs_uses_2026_start(self):
        with patch(
            "openai_monitor.get_costs_for_period", return_value=(42.0, None)
        ) as mock_p:
    ...
    def test_get_week_costs_start_is_monday(self):
        ...
        with patch(
            "openai_monitor.get_costs_for_period", return_value=(7.0, None)
        ) as mock_p:
    ...
    def test_get_month_costs_start_is_first_day(self):
        ...
        with patch(
            "openai_monitor.get_costs_for_period", return_value=(30.0, None)
        ) as mock_p:
    ...
    def test_get_last_weeks_costs_returns_n_weeks_reverse_order(self):
        with patch("openai_monitor.get_costs_for_period", return_value=(100.0, None)):
    ...
    def test_get_last_months_costs_returns_n_months(self):
        with patch("openai_monitor.get_costs_for_period", return_value=(50.0, None)):
...
class GetBillingBalanceTests(StateIsolationMixin, unittest.TestCase):
    ...
    def test_returns_none_on_network_error(self):
        with patch(
            "openai_monitor.requests.get", side_effect=requests.ConnectionError("no")
        ):
            self.assertIsNone(openai_monitor.get_billing_balance())
    ...
    def test_returns_none_when_fields_missing(self):
        with patch(
            ...
            return_value=_mock_response(
                200, {"total_granted": None, "total_used": None}
            ),
        ):
            self.assertIsNone(openai_monitor.get_billing_balance())
    ...
    def test_returns_balance_dict_on_success(self):
        with patch(
            "openai_monitor.requests.get",
            return_value=_mock_response(
                200, {"total_granted": "100.5", "total_used": "25.25"}
            ),
        ):
...
class LoadStateTests(StateIsolationMixin, unittest.TestCase):
    def test_load_state_fills_defaults_when_file_missing(self):
        ...
        self.assertEqual(
            state["total_deposited"], openai_monitor.DEFAULT_TOTAL_DEPOSITED
        )
...
class GetStatusMessageTests(StateIsolationMixin, unittest.TestCase):
    def test_manual_mode_formats_balance(self):
        ...
        with (
            patch("openai_monitor.get_total_costs", return_value=(200.0, None)),
    ...
    def test_billing_mode_uses_billing_values(self):
        with (
            patch("openai_monitor.get_total_costs", return_value=(100.0, None)),
            patch("openai_monitor.get_today_costs", return_value=2.0),
            patch(
                "openai_monitor.get_billing_balance",
                return_value={
                    "total_granted": 500.0,
                    "total_used": 123.0,
                    "remaining": 377.0,
    ...
    def test_manual_mode_caches_total_spent_on_success(self):
        # Свежий успешный ответ API должен сохранить total_spent в state —
        # чтобы следующий раз при падении API можно было показать кэш.
        self.write_state({"total_deposited": 1000, "alert_threshold": 50})
        with (
            patch("openai_monitor.get_total_costs", return_value=(200.0, None)),
            patch("openai_monitor.get_today_costs", return_value=5.0),
            ...
            openai_monitor.get_status_message()
        state = self.read_state()
        self.assertEqual(state["last_total_spent"], 200.0)
        self.assertIsNotNone(state["last_total_spent_at"])

    def test_manual_mode_uses_cache_when_api_fails(self):
        # Costs API упал → есть кэш → показываем остаток из кэша с меткой «кэш».
        ...
        self.write_state(
            {
                ...
                "alert_threshold": 50,
                "last_total_spent": 180.0,
                "last_total_spent_at": stale_at,
            }
        )
        with (
            patch("openai_monitor.get_total_costs", return_value=(None, "HTTP 500")),
            patch("openai_monitor.get_today_costs", return_value=None),
            patch("openai_monitor.get_billing_balance", return_value=None),
        ):
            msg = openai_monitor.get_status_message()
        self.assertIn("820", msg)  # 1000 - 180 = 820 из кэша
        self.assertIn("кэш", msg)  # маркер stale в строке «Остаток»
        self.assertIn("HTTP 500", msg)  # ошибка API тоже видна

    def test_manual_mode_no_cache_and_api_fails_returns_error(self):
        # API упал, кэша нет → честная ошибка, а не фиктивный полный баланс.
        self.write_state({"total_deposited": 1000, "alert_threshold": 50})
        with (
            patch("openai_monitor.get_total_costs", return_value=(None, "HTTP 500")),
            patch("openai_monitor.get_billing_balance", return_value=None),
            ...
            msg = openai_monitor.get_status_message()
        self.assertIn("Не удалось получить данные", msg)
        # Остатка-плейсхолдера быть не должно — сообщение короткое, только ошибка.
...
class CheckAndAlertTests(StateIsolationMixin, unittest.TestCase):
    def _run(
        ...
        with (
            patch("openai_monitor.get_total_costs", return_value=(total_spent, None)),
            patch("openai_monitor.get_today_costs", return_value=1.0),
            patch("openai_monitor.get_billing_balance", return_value=None),
            patch(
                "openai_monitor.send_telegram_alert", return_value=telegram_ok
            ) as mock_tg,
        ):
    ...
    def test_telegram_failure_raises_systemexit(self):
        ...
        self.write_state({"total_deposited": 1000, "alert_threshold": 100})
        with (
            patch("openai_monitor.get_total_costs", return_value=(950.0, None)),
            patch("openai_monitor.get_today_costs", return_value=1.0),
            patch("openai_monitor.get_billing_balance", return_value=None),
...
class SendStatusReportTests(StateIsolationMixin, unittest.TestCase):
    def _common_patches(self, remaining=100, forecast=10, chart_bytes=b"\x89PNG..."):
        ...
        return [
            ...
            patch(
                "openai_monitor.get_daily_costs",
                return_value=[(date_type(2026, 4, 18), 5.0)],
            ),
            patch(
                "openai_monitor.get_month_line_item_costs",
    ...
    def test_sends_chart_on_success(self):
        self.write_state(
            {"total_deposited": 150, "alert_threshold": 100, "topup_history": []}
        )
    ...
    def test_missing_env_aborts(self):
        openai_monitor.CONFIG["openai_admin_key"] = ""
        with (
            patch("openai_monitor.get_status_message") as mock_msg,
            patch("openai_monitor.get_total_costs") as mock_costs,
            patch("openai_monitor.send_telegram_alert") as mock_tg,
        ):
            openai_monitor.send_status_report()
    ...
    def test_costs_api_down_no_cache_sends_error_text(self):
        # API упал, кэша нет, billing недоступен → шлём текст с ошибкой,
        # а не график с фиктивным «остаток = всего внесено».
        ...
        self.assertIn("HTTP 500", msg)
        # Фиктивный «остаток = всего внесено» не должен просочиться.
        self.assertNotIn("7058.71", msg)

    def test_costs_api_down_with_cache_uses_stale_value(self):
        # API упал, но в state лежит last_total_spent → показываем его с меткой.
        from datetime import datetime, timezone, timedelta

        stale_at = (datetime.now(timezone.utc) - timedelta(hours=2)).isoformat()
        self.write_state(
            {
                "total_deposited": 7058.71,
                "alert_threshold": 200,
                "topup_history": [],
                "last_total_spent": 500.0,
                "last_total_spent_at": stale_at,
            }
        )
        with (
...
class MainRoutingTests(unittest.TestCase):
    def test_bot_flag_invokes_run_bot(self):
        with (
            patch("openai_monitor.run_bot") as mock_bot,
    ...
    def test_status_flag_invokes_status_report(self):
        with (
            patch("openai_monitor.send_status_report") as mock_st,
    ...
    def test_backup_flag_invokes_backup(self):
        with (
            patch("openai_monitor.backup_state") as mock_bk,
    ...
    def test_topup_flag_with_valid_amount(self):
        with (
            patch("openai_monitor.topup") as mock_tu,
    ...
    def test_no_flag_runs_check_and_alert(self):
        with (
            patch("openai_monitor.check_and_alert") as mock_ca,
...
class GetLineItemCostsTests(StateIsolationMixin, unittest.TestCase):
            ...
            "data": [
                {
                    "results": [
                        {
                            "line_item": li,
                            "amount": {"value": str(v), "currency": "usd"},
                        }
                        for li, v in items
                    ]
                }
    ...
    def test_aggregates_and_sorts_desc(self):
        p1 = self._page(
            [("gpt-5.2, input", 10), ("whisper-1, audio", 3), ("gpt-5.2, input", 5)]
        )
        resp = MagicMock()
        resp.status_code = 200
        resp.json.return_value = p1
        with patch("openai_monitor.requests.get", return_value=resp):
...
class ParseTopupDateTests(unittest.TestCase):
    def test_actual_datetime_with_time(self):
        d = openai_monitor._parse_topup_date({"actual_datetime": "01.02.2026 14:30"})
...
class BackupStateTests(StateIsolationMixin, unittest.TestCase):
    def test_sends_state_file_as_document(self):
        self.write_state({"total_deposited": 100})
        with patch(
            "openai_monitor.send_telegram_document", return_value=True
        ) as mock_send:
